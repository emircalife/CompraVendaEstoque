...
const
  cSQLConsultar : String = 'select vda.idvenda,                                              '+
                           '       vda.cliente,                                              '+
                           '       vda.formaPagamento,                                       '+
                           '       DATE_FORMAT(vda.dtVenda, ''%d/%m/%Y'')  as dtVenda,       '+
                           '       DATE_FORMAT(vda.dtEntrega, ''%d/%m/%Y'')  as dtEntrega,   '+
                           '       DATE_FORMAT(vda.dtCadastro, ''%d/%m/%Y'')  as dtCadastro, '+
                           '       vda.usuario,                                              '+
                           '       vda.status,                                               '+
                           '       cli.nomeCliente,                                          '+
                           '       forma.nomeFormaPagamento,                                 '+
                           '       stvda.nomeStatusVenda                                     '+
                           '  FROM estoque.vendas vda                                        '+
                           '  INNER JOIN estoque.clientes cli                                '+
                           '            ON (vda.cliente = cli.idCliente)                     '+
                           '  INNER JOIN estoque.formasPagamento forma                       '+
                           '            ON (vda.formaPagamento = forma.idFormaPagamento)     '+
                           '  INNER JOIN estoque.statusVendas stvda                          '+
                           '             ON (vda.status = stvda.idStatusVenda)               '+
...
const
  cSQLConsultar : String = 'SELECT vdi.idVendaItem,                                 '+
                           ...
                           '        pro.nomeProduto                                 '+
                           '  FROM estoque.vendaItens vdi                           '+
                           '  INNER JOIN estoque.categorias cat                     '+
                           '            ON (vdi.categoria = cat.idCategoria)        '+
                           '  INNER JOIN estoque.subcategorias scat                 '+
                           '            ON (vdi.subCategoria = scat.idSubcategoria) '+
                           '  INNER JOIN estoque.produtos pro                       '+
                           '            ON (vdi.produto = pro.idProduto)            '+
...
const
  cSQLManutencao : String = 'INSERT INTO estoque.compras (             '+
...
const
  cSQLManutencao : String = 'UPDATE estoque.compras SET          '+
...
const
  cSQLManutencao : String = 'INSERT INTO estoque.compraItens (        '+
...
const
  cSQLManutencao : String = 'UPDATE estoque.compraItens SET '+
                            ' compra =:Pcompra,             '+
