...
const
  cSQLConsultar : String = 'select cpa.idcompra,                                          '+
                           ...
                           '  FROM estoque.compras cpa                                    '+
                           '  INNER JOIN estoque.fornecedores forn                        '+
                           '             ON (cpa.fornecedor = forn.idFornecedor)          '+
                           '  INNER JOIN estoque.formasPagamento forma                    '+
                           '             ON (cpa.formaPagamento = forma.idFormaPagamento) '+
                           '  INNER JOIN estoque.statusCompras stcpa                      '+
                           '              ON (cpa.status = stcpa.idStatusCompra)          '+
...
const
  cSQLConsultar : String = 'SELECT cpi.idCompraItem,                                 '+
                           ...
                           '        pro.nomeProduto                                  '+
                           '  FROM estoque.compraItens cpi                           '+
                           '  INNER JOIN estoque.categorias cat                      '+
                           '             ON (cpi.categoria = cat.idCategoria)        '+
                           '  INNER JOIN estoque.subcategorias scat                  '+
                           '             ON (cpi.subCategoria = scat.idSubcategoria) '+
                           '  INNER JOIN estoque.produtos pro                        '+
                           '             ON (cpi.produto = pro.idProduto)            '+
